...
function actionInscription() {
   ...
   if (empty($email)) {
       ...
       foreach (DOMAINES_EMAIL_AUTORISES as $domaine) {
           if (estSuffixeEmail($email, '@' . $domaine)) {
               $domaineValide = true;
               break;
           }
       }
   ...
   $tokenVerification = genererToken(32);
...
function actionConnexion() {
   ...
   if (!verifierMotDePasse($motDePasse, $utilisateur['mot_de_passe'])) {
       repondreJSON(['success' => false, 'message' => 'Email ou mot de passe incorrect']);
       return;
   }
   ...
   repondreJSON([
       ...
       'redirection' => estAdmin() ? URL_SITE . '/admin' : URL_SITE
   ]);
}
...
function actionModifierProfil() {
   ...
   $idUtilisateur = getSessionValue('utilisateur_id');
...
function actionModifierMotDePasse() {
   ...
   if (!verifierMotDePasse($ancienMotDePasse, $utilisateur['mot_de_passe'])) {
       repondreJSON(['success' => false, 'message' => 'Ancien mot de passe incorrect']);
       return;
   }
   
   // Hacher le nouveau mot de passe
   $nouveauMotDePasseHash = hasherMotDePasse($nouveauMotDePasse);
...
*/
function estSuffixeEmail($chaine, $suffixe) {
   return substr_compare($chaine, $suffixe, -strlen($suffixe)) === 0;
}

/**
* Vérifie si un téléphone est au format français valide
* 
* @param string $telephone Numéro de téléphone à vérifier
* @return boolean True si le téléphone est valide, false sinon
*/
...
* 
* @param int $longueur Longueur du token à générer
...
/**
* Met à jour les données de session d'un utilisateur
* 
* @param array $donnees Données de session à mettre à jour
* @return void
*/
function mettreAJourSessionUtilisateur($donnees) {
...
* 
* @param string $cle Clé de la valeur à récupérer
* @param mixed $defaut Valeur par défaut si la clé n'existe pas
* @return mixed Valeur de session ou valeur par défaut
*/
function getSessionValue($cle, $defaut = null) {
