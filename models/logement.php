...
function modifierLogement($idLogement, $donnees) {
   if (!is_numeric($idLogement) || $idLogement <= 0) {
       return false;
   }
...
function recupererLogementsPropriete($idProprietaire) {
   if (!is_numeric($idProprietaire) || $idProprietaire <= 0) {
       return [];
   }
...
function logementEstDisponible($idLogement, $dateDebut, $dateFin) {
   if (!is_numeric($idLogement) || $idLogement <= 0 || !validateDate($dateDebut) || !validateDate($dateFin)) {
       return false;
   }
...
function recupererLogements($filtres = [], $tri = 'prix_asc', $limite = 0, $offset = 0) {
   ...
   if (isset($filtres['date_debut']) && !empty($filtres['date_debut']) && isset($filtres['date_fin']) && !empty($filtres['date_fin'])) {
       ...
       if (validateDate($filtres['date_debut']) && validateDate($filtres['date_fin'])) {
   ...
   if ($limite > 0) {
       // Préparation de la requête avec LIMIT et OFFSET
       $sql .= "LIMIT :limite OFFSET :offset";
       
       // Conversion en entiers pour éviter les injections SQL
...
function recupererPhotoParId($idPhoto) {
   if (!is_numeric($idPhoto) || $idPhoto <= 0) {
       return false;
   }
...
function recupererSuggestionsVilles($terme) {
   if (empty($terme) || !is_string($terme) || strlen($terme) < 2) {
       return [];
   }
