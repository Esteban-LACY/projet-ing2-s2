...
if ($stmt) {
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        if (password_verify($password, $user['password'])) {
            ...
            if ($_SESSION['is_admin']) {
                // Redirection vers la page d'administration pour les administrateurs
                header('Location: admin.php');
            } else {
                // Redirection vers la page d'accueil ou la page demandée pour les utilisateurs standard
    ...
    header('Location: connexion.php?login_error=Erreur de connexion à la base de données');
    exit;
}
?>